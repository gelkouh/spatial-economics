...
econ_indicators_df <- read_csv(file.path(ddir,"Analysis Files", "CAINC30", "CAINC30_CA_1969_2018.csv")) %>% # From BEA (economic profile)
  ...
  select(-c("2012","2013","2014","2015","2016","2017","2018")) %>%
  select(-c("GeoFIPS"))
...
zillow_county_for_sale_listings_df <- zillow_county_for_sale_listings_df %>%
  select(-contains(c("-02","-03","-04","-05","-06","-07","-08","-09","-10","-11","-01"))) %>%
  select(-contains(c("2012","2013","2014","2015","2016","2017","2018","2019","2020")))
colnames(zillow_county_for_sale_listings_df) <- c(1,2,"COUNTY",1990:2011)
zillow_county_for_sale_listings_df <- zillow_county_for_sale_listings_df %>%
  select(c("COUNTY","1996","1997","1998","1999","2000","2001","2002","2003","2004","2005","2006","2007","2008","2009","2010","2011")) %>%
  reshape2::melt(id=c("COUNTY")) %>%
  dplyr::rename("YEAR"="variable") %>%
...
intersect_df <- intersect_df %>%
  ...
  select(-c("GEOID10","ALAND10","AWATER10"))
...
zcta_2019_df <- merge(zcta_2019_df,zipcodes_by_state_df,by="ZCTA_2019",all.x=TRUE) %>%
  filter(state_id == "CA") %>%
  select(c("ZCTA_2019","ZCTA_SHAPE_AREA","POPULATION"="population","COUNTY"="county_name"))
...
panel_df <- merge(county_weighted_percentage_df,econ_indicators_df,by = c("COUNTY","YEAR"), all.y = TRUE) %>%
  select(-c("ZCTA","HOUSING_UNIT_QUANTITY_CENSUS","ZCTA_POP","COUNTY_POP","ALARM_DATE","CONT_DATE","FIRE_LENGTH_DAYS","FIRE_NAME","PERCENT_ZCTA_BURNED","ZCTA_BY_COUNTY_WEIGHT")) %>%
